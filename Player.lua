...
function Player:init(map)
    ...
    self.sounds = {
        ...
        ['kill'] = love.audio.newSource('sounds/Explosion6.wav', 'static'),
    ...
    self.behaviors = {
        ...
        ['jumping'] = function(dt)
            ...
            if self.y > 300 then   -- TODO
                os.exit()                                                                          -- FOR NOW JUST EXIT, TODO RESET FUNCTION WITH CONDIDIONS IF WANNA RESTART
            end
...
function Player:update(dt)
    ...
    if self.x >=  self.map.flag then        -- TODO PLACE A FRLAG
        self:winning()
    end
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id ~= TILE_EMPTY or
            ...
            if playCoin then
                self.sounds['coin']:play(0.01)
            elseif playHit then
                self.sounds['hit']:play(0.01)
            end
        end
    end
end
...
function Player:render()
    ...
    if self.result == 'win' then
        ...
        love.graphics.printf("You Won!", 0, 20, math.floor((map.camX + VIRTUAL_WIDTH / 2) * 2), 'center')
    end
...
function Player:winning()                                                                          -- JUST PRINTS A WIN MESSAGE
    self.result = 'win'
end
